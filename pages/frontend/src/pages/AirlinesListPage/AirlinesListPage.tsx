...
const AirlinesListPage = ({airlines, setAirlines, isMock, setIsMock}:Props) => {
    ...
    const {airline_name} = useAppSelector((state) => state.airlines)
    ...
    const fetchAirlines = async () => {
        try {
            const response = await fetch(`http://localhost:8000/api/airlines/?airline_name=${airline_name.toLowerCase()}`)
